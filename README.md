...
* **In-Memory Store & Thread Safety**: Cart items are stored in nested Go maps (`carts map[string]map[uuid.UUID]int`) guarded by `sync.RWMutex`. Multiple HTTP readers (`GetCart`) acquire read locks (`RLock()`) concurrently, while mutating operations (`AddCartItem`, `UpdateCartItem`, `RemoveCartItem`, `ClearCart`) acquire exclusive write locks (`Lock()`). To prevent lock contention and iteration data races during outgoing HTTP calls to `Product API`, item keys and quantities are snapshotted locally under `RLock()` before network I/O is performed.
...
* **Concurrent Reads/Writes & Race Safety**: Built-in concurrency testing (`TestCartHandler_ConcurrentReadsWrites`) verifies parallel read/write handling under `go test -race`. Memory safety is enforced by snapshotting map items under `sync.RWMutex` read locks before initiating remote network calls. Even without product deletions, domain-level races can still occur in this microservice setup:
  - **Lost Updates (Read-Modify-Write Race)**: If a client fires concurrent `AddCartItem` and `UpdateCartItem` requests, network latency can cause out-of-order execution, overwriting quantity updates without optimistic concurrency control (`ETag` or versioning headers).
  - **Dynamic Price Resolution Window**: Cart items store product IDs and quantities, resolving prices on-the-fly from `Product API`. If a product price is updated in `Product API` mid-session, concurrent `GetCart` calls can return different totals before the user checks out.
  - **Process Boundary Race (Horizontal Scaling)**: Storing state in single-process Go maps means scaling `Cart API` to multiple instances behind a load balancer without sticky sessions causes requests to hit different pods, resulting in inconsistent cart reads.
